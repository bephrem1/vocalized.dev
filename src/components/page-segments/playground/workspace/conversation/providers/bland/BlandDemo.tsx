...
const BlandDemo: FunctionComponent<BlandDemoProps> = ({ index }) => {
  ...
  const disabled = useConvoDemoDisabled({ providerId: Projects.Bland.id });
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className={clsx({ 'opacity-50': disabled })}>
        ...
        <ConvoDemoLinks docsLink={Projects.Bland.siteLinks.documentation} />
        <ConvoDemoLogoSymbol src={Projects.Bland.logo.localPath} />
      </div>
    </div>
  );
};
  ...
  useEffect(() => {
    if (blandClient) {
      blandClient.on('conversationStarted', () => {
        ...
        resetLatencyTimer();
        clearLatencyReadings();
        setActiveConvoProviderId(Projects.Bland.id);
      });
...
const useOnClick = ({ modelId, voiceId, callState, setCallState, blandClient, setBlandClient }) => {
  ...
  const startCall = async () => {
    const credentials = getCredentials({ providerId: Projects.Bland.id });
