...
const VapiDemo: FunctionComponent<VapiDemoProps> = ({ index }) => {
  ...
  const disabled = useConvoDemoDisabled({ providerId: Projects.Vapi.id });
  ...
  return (
    <div className={className}>
      ...
      <div className={clsx({ 'opacity-50': disabled })}>
        ...
        <ConvoDemoLinks docsLink={Projects.Vapi.siteLinks.documentation} />
        <ConvoDemoLogoSymbol src={Projects.Vapi.logo.localPath} />
      </div>
    </div>
  );
};
...
const useVapi = ({
  ...
  const credentials = getCredentials({ providerId: Projects.Vapi.id });
  ...
  useEffect(() => {
    if (vapiClient) {
      vapiClient.on('call-start', () => {
        ...
        resetLatencyTimer();
        clearLatencyReadings();
        setActiveConvoProviderId(Projects.Vapi.id);
      });
