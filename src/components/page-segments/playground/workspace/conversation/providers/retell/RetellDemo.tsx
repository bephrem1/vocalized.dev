...
const RetellDemo: FunctionComponent<RetellDemoProps> = ({ index }) => {
  ...
  const disabled = useConvoDemoDisabled({ providerId: Projects.Retell.id });
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className={clsx({ 'opacity-50': disabled })}>
        ...
        <ConvoDemoLinks docsLink={Projects.Retell.siteLinks.documentation} />
        <ConvoDemoLogoSymbol src={Projects.Retell.logo.localPath} />
      </div>
    </div>
  );
};
...
const useRetell = ({
  ...
  RetellClient.on('conversationStarted', () => {
    ...
    resetLatencyTimer();
    clearLatencyReadings();
    setActiveConvoProviderId(Projects.Retell.id);
  });
...
const useOnClick = ({ modelId, voiceId, sampleRate, callState, setCallState }) => {
  ...
  const startCall = async () => {
    const credentials = getCredentials({ providerId: Projects.Retell.id });
